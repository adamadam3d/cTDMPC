...
class TDMPC2(torch.nn.Module):
	...
	def _update(self, obs, action, reward, terminated, task=None, ctx=None):
		...
		if self.cfg.multitask:
			# Belief diagnostics: average posterior std (-> 0 as the task becomes
			# identifiable, -> 1 as the belief falls back to the prior) and the
			# typical magnitude of the belief mean. z_ctx is the belief [mu, logvar].
	...
	def _model_loss(self, obs, action, reward, terminated, task, ctx):
		...
		reward, value, sequential belief KL) but performs no optimiser or policy
		update and runs eagerly (it is never compiled). The termination term is
		omitted; it is inactive for the non-episodic multitask suites this is
		used on.
		"""
		beliefs = self.model.belief_rollout(ctx)
		t_idx = torch.randint(0, beliefs.shape[1], (beliefs.shape[0],), device=beliefs.device)
