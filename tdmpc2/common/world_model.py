...
class WorldModel(nn.Module):
	...
	def __init__(self, cfg):
		...
		self.cfg = cfg
		if cfg.multitask:
			# Context encoder over (s, a, r, s') tuples, selected by cfg.context_encoder:
			#   pearl      -- per-tuple Gaussian factors combined by a Product of
			#                 Gaussians into a posterior q(z|c) (permutation-invariant).
			#   varibad    -- a GRU processes tuples in temporal order and emits a
	...
	def infer_ctx(self, ctx, mask=None):
		"""
		Infers the task latent from a set of context transitions (s, a, r, s').
		Used by the permutation-invariant encoders (the VariBAD encoder uses the
		recurrent `belief_rollout` / `belief_update` instead):
		  pearl      -- returns the posterior q(z|c) as (mean, log-variance),
		                each of shape (..., task_dim), via a Product of Gaussians.
		  supervised -- returns a deterministic latent z of shape (..., task_dim)
		                by mean-pooling the per-tuple embeddings.
		...
		# supervised: deterministic mean-pool
	...
	def belief_update(self, tup, h):
		"""
		One-step recurrent belief update for online inference (VariBAD).
		...
		Returns:
			tuple: Updated belief [mu, logvar] of shape (B, task_dim), and the
				new hidden state.
		"""
