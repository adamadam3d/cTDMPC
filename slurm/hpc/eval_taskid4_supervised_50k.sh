#!/bin/bash
#SBATCH --job-name=tdmpc2-eval-mixed
...
# Evaluate every 50k checkpoint of a fixed list of (encoder, seed, project)
# combos in one submission:
#
#     sbatch slurm/hpc/eval_taskid4_supervised_50k.sh
#
# Combos (one per COMBOS entry, in order):
#   0) task_id    seed 4  -> wandb project eval50k
#   1) supervised seed 7  -> wandb project supervised_eval
#   2) supervised seed 4  -> wandb project supervised_eval
#   3) supervised seed 5  -> wandb project supervised_eval
#   4) supervised seed 3  -> wandb project supervised_eval
#
# 20 array tasks = 5 combos x 4 GPUs. Each task takes one GPU and packs 4 eval
# processes onto it (eval is MuJoCo-bound, the GPU is mostly idle), so each
# combo's ~200 checkpoints are split into 16 disjoint shards of ~13.
#
# Metrics go to the combo's wandb project (one group per exp_name, x-axis
# iteration) AND to
# logs/mt30/<seed>/<project>_seed<seed>_<encoder>_param5/metrics_shard*.csv.
...
#          --export=ALL,PROCS_PER_GPU=8 slurm/hpc/eval_taskid4_supervised_50k.sh
# (CLI flags override the #SBATCH directives above.) NOTE: num_shards depends
# on this value, so do not mix packings within one combo at the same time --
...
#   sbatch --export=ALL,USE_MPS=1 slurm/hpc/eval_taskid4_supervised_50k.sh
